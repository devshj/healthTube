...
// 페이징
$query = "select * from Heal_video $searchText $searchBody $searchEquipment order by idx desc";
$conn->DBQ($query);
$conn->DBE();
$cnt = $conn->resultRow();
...
<section class="blog_area section_padding" style="min-height:1100px;padding: 100px 0px;">
	<div class="container">
		...
		<div class="row">
			...
			<?
			$sql = "
			select * from Heal_video $searchText $searchBody $searchEquipment order by idx desc limit $limit, $list;
			";
			$conn->DBQ($sql);
			$conn->DBE();
			while($row=$conn->DBF()){
